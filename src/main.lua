function love.load()
	...
	stepx = width/80
	stepy = height/25
	zombies = {}--Массив зомби:)
	zombies.max = 5--Mаксимальное колиество зомби
	zombies.zombik = {}
	wall = {}
	inition_walls()
...
function zombik()
	for i = 1, zombies.max do
	zombies.zombik[i] = create_zombies()
	end
	for i = 1, zombies.max do
	draw_zombies(zombies.zombik[i].x,zombies.zombik[i].y)
	end
end
...
function draw_zombies(locstepx,locstepy)--Рисуем зомби
	love.graphics.setColor( 0, 255, 0, 255 )
	love.graphics.rectangle("fill", locstepx*40+2, locstepy*14+2, stepx-4, stepy-4)
end
